...
WinMenu::WinMenu(sf::Texture* playAgainTexture, sf::Texture* menuTexture) :
	playAgainAnimation(playAgainTexture), MenuAnimation(menuTexture)
{
	this->mouseOnPlay = false;
	this->mouseOnExit = false;

	backgroundImage.loadFromFile("./Assets/Images/WinMenuBackground.png");
	...
	sf::Vector2f playSize = sf::Vector2f(71.0f, 24.0f) * 2.25f;
	playAgainButton.setSize(playSize);
	playAgainButton.setOrigin(playSize / 2.0f);
	playAgainButton.setPosition(sf::Vector2f(-100.0f, 80.0f));
	...
	MenuButton.setPosition(sf::Vector2f(85.0f, 80.0f));

	MenuButton.setTexture(menuTexture);
}

void WinMenu::Update(sf::Vector2f mousePos)
{
